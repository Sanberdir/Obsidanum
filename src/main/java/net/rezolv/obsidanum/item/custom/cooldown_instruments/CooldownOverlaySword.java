...
public class CooldownOverlaySword {
    ...
    public static final IGuiOverlay COOLDOWN_OVERLAY = (gui, guiGraphics, partialTick, screenWidth, screenHeight) -> {
        ...
        if (mainHandItem.getItem() instanceof ObsidanSword) {
            swordStack = mainHandItem;
            inMain = true;
        } else if (offHandItem.getItem() instanceof ObsidanSword) {
            swordStack = offHandItem;
            inMain = false;
        }

        if (swordStack != null) {
            long currentTime = minecraft.level.getGameTime();
            long cooldownEnd = swordStack.getOrCreateTag().getLong("CooldownEndTime");
