...
public class OrderJeiRecipe implements IRecipeCategory<ForgeScrollOrderRecipe> {
    public static final ResourceLocation UID = new ResourceLocation(Obsidanum.MOD_ID, "scrolls/forge_scroll_order");
    public static final ResourceLocation TEXTURE = new ResourceLocation(Obsidanum.MOD_ID,
            "textures/gui/forge_crucible_menu.png");

    public static final RecipeType<ForgeScrollOrderRecipe> ORDER_JEI_RECIPE_TYPE =
            new RecipeType<>(new ResourceLocation(Obsidanum.MOD_ID, "forge_scroll_order"), ForgeScrollOrderRecipe.class);

    private final IDrawable background;
    private final IDrawable icon;
    private final Font font;

    public OrderJeiRecipe(IGuiHelper helper) {
        this.background = helper.createDrawable(TEXTURE, 39, 0, 176, 130);
        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(ItemsObs.ORDER_PLAN.get()));
        this.font = Minecraft.getInstance().font;
    }

    @Override
    public RecipeType<ForgeScrollOrderRecipe> getRecipeType() {
        return ORDER_JEI_RECIPE_TYPE;
    }

    @Override
    public void setRecipe(IRecipeLayoutBuilder builder,
                          ForgeScrollOrderRecipe recipe,
                          IFocusGroup focuses) {
        // Основной результат
    ...
    public Component getTitle() {
        return Component.translatable("block.obsidanum.order_jei_recipe");
    }
    ...
    @Override
    public void draw(ForgeScrollOrderRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
        ItemStack diamondStack = new ItemStack(ItemsObs.ORDER_PLAN.get());
