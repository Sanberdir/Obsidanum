...
public class CatacombJeiRecipe implements IRecipeCategory<ForgeScrollCatacombsRecipe> {
    public static final ResourceLocation UID = new ResourceLocation(Obsidanum.MOD_ID, "scrolls/forge_scroll_catacombs");
    public static final ResourceLocation TEXTURE = new ResourceLocation(Obsidanum.MOD_ID,
            "textures/gui/forge_crucible_menu.png");

    public static final RecipeType<ForgeScrollCatacombsRecipe> CATACOMB_JEI_RECIPE_TYPE =
            new RecipeType<>(new ResourceLocation(Obsidanum.MOD_ID, "forge_scroll_catacombs"), ForgeScrollCatacombsRecipe.class);

    private final IDrawable background;
    private final IDrawable icon;
    private final Font font;

    public CatacombJeiRecipe(IGuiHelper helper) {
        this.background = helper.createDrawable(TEXTURE, 39, 0, 176, 130);
        this.icon = helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(ItemsObs.CATACOMBS_PLAN.get()));
        this.font = Minecraft.getInstance().font;
    }

    @Override
    public RecipeType<ForgeScrollCatacombsRecipe> getRecipeType() {
        return CATACOMB_JEI_RECIPE_TYPE;
    }

    @Override
    public void setRecipe(IRecipeLayoutBuilder builder,
                          ForgeScrollCatacombsRecipe recipe,
                          IFocusGroup focuses) {
        // Основной результат
    ...
    public Component getTitle() {
        return Component.translatable("block.obsidanum.catacombs_jei_recipe");
    }
    ...
    @Override
    public void draw(ForgeScrollCatacombsRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
        ItemStack diamondStack = new ItemStack(ItemsObs.CATACOMBS_PLAN.get());
